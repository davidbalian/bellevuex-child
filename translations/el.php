<?php
return [

	/* ── Common UI / CTAs ─────────────────────────────────────────────── */
	'Book Now'                  => 'Κράτηση',
	'View'                      => 'Προβολή',
	'View Photos'               => 'Δείτε Φωτογραφίες',
	'Suite'                     => 'Σουίτα',
	'Previous photo'            => 'Προηγούμενη φωτογραφία',
	'Next photo'                => 'Επόμενη φωτογραφία',
	'Previous suites'           => 'Προηγούμενες σουίτες',
	'Next suites'               => 'Επόμενες σουίτες',
	'Primary'                   => 'Κύρια',
	'Open menu'                 => 'Άνοιγμα μενού',
	'Return to Homepage'        => 'Επιστροφή στην Αρχική',

	/* ── Navigation ──────────────────────────────────────────────────── */
	'Home'                      => 'Αρχική',
	'Explore Athens'            => 'Εξερευνήστε την Αθήνα',
	'Buildings'                 => 'Κτίρια',
	'Reviews'                   => 'Κριτικές',
	'Signup / Login'            => 'Εγγραφή / Σύνδεση',

	/* ── Language switcher ───────────────────────────────────────────── */
	'Switch to English'         => 'Αλλαγή σε Αγγλικά',
	'Switch to Greek'           => 'Αλλαγή σε Ελληνικά',

	/* ── Capacity / amenities ────────────────────────────────────────── */
	'Up to 2 guests'            => 'Έως 2 άτομα',
	'Up to 4 guests'            => 'Έως 4 άτομα',
	'Hosting up to %d guests'   => 'Φιλοξενεί έως %d άτομα',
	'King Size Bed'             => 'Κρεβάτι King Size',
	'Equipped Kitchen'          => 'Εξοπλισμένη Κουζίνα',
	'Jacuzzi'                   => 'Τζακούζι',
	'Balcony'                   => 'Μπαλκόνι',
	'Terrace'                   => 'Βεράντα',
	'Shower'                    => 'Ντους',
	'Sofa'                      => 'Καναπές',
	'Sofa / Bed'                => 'Καναπές / Κρεβάτι',
	'Double Bed'                => 'Διπλό Κρεβάτι',

	/* ── Building short labels ───────────────────────────────────────── */
	'Thiseos 11'                => 'Θησέως 11',
	'Thiseos 13'                => 'Θησέως 13',
	'Chavriou 2'                => 'Χαβρίου 2',
	'11 Thiseos, Floor 1, 10562 Athens' => 'Θησέως 11, 1ος Όροφος, 10562 Αθήνα',
	'13 Thiseos, Floor 4, 10562 Athens' => 'Θησέως 13, 4ος Όροφος, 10562 Αθήνα',
	'2 Chavriou, Floor 2, 10562 Athens' => 'Χαβρίου 2, 2ος Όροφος, 10562 Αθήνα',

	/* ── Hero fallback alt texts ─────────────────────────────────────── */
	'Chic Centre Suites Athens suite interior'      => 'Εσωτερικό σουίτας στα Chic Centre Suites Αθήνα',
	'Chic Centre Suites Athens nearby attractions'  => 'Αξιοθέατα κοντά στα Chic Centre Suites Αθήνα',
	'Chic Centre Suites Athens deluxe suite kitchen' => 'Κουζίνα deluxe σουίτας στα Chic Centre Suites Αθήνα',

	/* ── Home page ───────────────────────────────────────────────────── */
	'Chic Centre Suites hero photos'        => 'Φωτογραφίες από τα Chic Centre Suites',
	'Stay in the Heart<br> of Athens'       => 'Ζήστε στην<br> Καρδιά της Αθήνας',
	'Our suites, spread across three buildings on the same road — steps from Syntagma Square, Monastiraki, and the Ermou pedestrian street.'
		=> 'Οι σουίτες μας βρίσκονται σε τρία κτίρια στον ίδιο δρόμο — βήματα από την Πλατεία Συντάγματος, το Μοναστηράκι και την πεζόδρομο Ερμού.',
	'Scroll to next section'                => 'Κύλιση στην επόμενη ενότητα',
	'Our Suites'                            => 'Οι Σουίτες μας',
	'Our fully equipped apartments are located in three buildings on the same road in the heart of Athens, just a 5-minute walk from Syntagma Square and Monastiraki, and only steps away from the iconic pedestrian street of Ermou, where you can explore the city&rsquo;s treasures, experience its culture and history, and, of course, enjoy shopping.'
		=> 'Τα πλήρως εξοπλισμένα διαμερίσματά μας βρίσκονται σε τρία κτίρια στον ίδιο δρόμο, στην καρδιά της Αθήνας — μόλις 5 λεπτά με τα πόδια από το Σύνταγμα και το Μοναστηράκι, και δίπλα στον εμβληματικό πεζόδρομο της Ερμού, όπου μπορείτε να εξερευνήσετε την πόλη, να βιώσετε την ιστορία και τον πολιτισμό της και, φυσικά, να απολαύσετε ψώνια.',

	/* ── 404 page ────────────────────────────────────────────────────── */
	'404 error'                             => 'Σφάλμα 404',
	'404 Error'                             => 'Σφάλμα 404',
	'Page Not Found'                        => 'Η Σελίδα Δεν Βρέθηκε',
	"The page you're looking for doesn't exist or may have been moved."
		=> 'Η σελίδα που ψάχνετε δεν υπάρχει ή έχει μεταφερθεί.',

	/* ── Cookie banner ───────────────────────────────────────────────── */
	'Cookie consent'            => 'Συγκατάθεση cookies',
	'Dismiss cookie banner'     => 'Κλείσιμο ειδοποίησης cookies',
	'We value your privacy'     => 'Σεβόμαστε την ιδιωτικότητά σας',
	'We use cookies to analyse site traffic and improve your experience.'
		=> 'Χρησιμοποιούμε cookies για την ανάλυση της επισκεψιμότητας και τη βελτίωση της εμπειρίας σας.',
	'Privacy Policy'            => 'Πολιτική Απορρήτου',
	'See our %s for details.'   => 'Δείτε την %s για περισσότερες πληροφορίες.',
	'Accept'                    => 'Αποδοχή',
	'Reject'                    => 'Απόρριψη',

	/* ── Footer ──────────────────────────────────────────────────────── */
	'Contact Info'              => 'Στοιχεία Επικοινωνίας',
	'Useful Links'              => 'Χρήσιμοι Σύνδεσμοι',
	'Awards'                    => 'Βραβεία',
	'Follow Us'                 => 'Ακολουθήστε μας',
	'Instagram'                 => 'Instagram',
	'Facebook'                  => 'Facebook',
	'Cookie Policy'             => 'Πολιτική Cookies',
	'Terms &amp; Conditions'    => 'Όροι &amp; Προϋποθέσεις',
	'All rights reserved.'      => 'Με επιφύλαξη παντός δικαιώματος.',
	'Developed by'              => 'Ανάπτυξη ιστοσελίδας από',

	/* ── Single suite page ───────────────────────────────────────────── */
	'Suite Description'         => 'Περιγραφή Σουίτας',
	'Photos'                    => 'Φωτογραφίες',
	'Suite Facilities'          => 'Παροχές Σουίτας',
	'Free Wi-Fi'                => 'Δωρεάν Wi-Fi',
	'Fully Equipped Kitchen included Nespresso coffee machine'
		=> 'Πλήρως εξοπλισμένη κουζίνα με μηχανή Nespresso',
	'Towels, bath robes, slippers'  => 'Πετσέτες, μπουρνούζια, παντόφλες',
	'USB-A Sockets'             => 'Πρίζες USB-A',
	'Hangers'                   => 'Κρεμάστρες',
	'Easy keyless secure access'    => 'Εύκολη και ασφαλής πρόσβαση χωρίς κλειδί',
	'Elevator in building'      => 'Ανελκυστήρας στο κτίριο',
	'Safe box'                  => 'Χρηματοκιβώτιο',
	'55″ Smart TV with Netflix'     => '55″ Smart TV με Netflix',
	'Air Conditioning'          => 'Κλιματισμός',
	'Iron with iron board'      => 'Σίδερο με σανίδα σιδερώματος',
	'Hair dryer'                => 'Πιστολάκι μαλλιών',
	'Welcome consumables upon arrival'  => 'Αναλώσιμα καλωσορίσματος κατά την άφιξη',
	'High Quality Mattress &amp; Pillows'   => 'Στρώμα &amp; Μαξιλάρια Υψηλής Ποιότητας',
	'High Quality Mattress & Pillows'   => 'Στρώμα & Μαξιλάρια Υψηλής Ποιότητας',
	'Smoke detector &amp; extinguisher'     => 'Ανιχνευτής καπνού &amp; πυροσβεστήρας',
	'Smoke detector & extinguisher'     => 'Ανιχνευτής καπνού & πυροσβεστήρας',
	'Ambient lighting'          => 'Διακριτικός φωτισμός περιβάλλοντος',
	'First Aid Kit'             => 'Κουτί Πρώτων Βοηθειών',
	'Linen'                     => 'Λευκά είδη',
	'Cosmetics'                 => 'Προϊόντα περιποίησης',
	'Soap, shampoo, shower gel'     => 'Σαπούνι, σαμπουάν, αφρόλουτρο',
	'Hygiene:'                  => 'Καθαριότητα:',
	'Fully cleaned and disinfected suite upon your arrival.'
		=> 'Η σουίτα παραδίδεται πλήρως καθαρή και απολυμασμένη κατά την άφιξή σας.',
	'Room Cleaning Service:'    => 'Υπηρεσία Καθαρισμού:',
	'Towels will be changed every 4 days.'
		=> 'Οι πετσέτες αλλάζουν κάθε 4 ημέρες.',
	'However, we also offer the following options for your convenience:'
		=> 'Επίσης, για τη διευκόλυνσή σας, παρέχουμε τις εξής επιλογές:',
	'Room Cleaning:'            => 'Καθαρισμός δωματίου:',
	'Available upon request for an additional cost of €30. This service includes general cleaning of the room, fresh bed sheets, and towels.'
		=> 'Διαθέσιμος κατόπιν αιτήματος με επιπλέον χρέωση €30. Η υπηρεσία περιλαμβάνει γενικό καθαρισμό, αλλαγή σεντονιών και πετσετών.',
	'Extra Towels:'             => 'Επιπλέον Πετσέτες:',
	'Guests may request an additional set of fresh bath towels for an extra cost of €5 per set.'
		=> 'Οι επισκέπτες μπορούν να ζητήσουν επιπλέον σετ μπάνιου με επιπλέον χρέωση €5 ανά σετ.',

	/* ── Explore Athens page ─────────────────────────────────────────── */
	'The Acropolis of Athens at sunset, overlooking the city'
		=> 'Η Ακρόπολη της Αθήνας στο ηλιοβασίλεμα με θέα στην πόλη',
	'EXPLORE AND<br>EXPERIENCE ATHENS'
		=> 'ΕΞΕΡΕΥΝΗΣΤΕ<br>ΤΗΝ ΑΘΗΝΑ',
	"Discover the timeless beauty of Greece's capital from the heart of the city"
		=> 'Ανακαλύψτε τη διαχρονική ομορφιά της ελληνικής πρωτεύουσας από την καρδιά της',
	'Explore Athens / Athina'   => 'Εξερευνήστε την Αθήνα',
	'A City Steeped in History, Culture, and Ancient Landmarks'
		=> 'Μια Πόλη Γεμάτη Ιστορία, Πολιτισμό και Αρχαία Μνημεία',
	'Staying at Athens means immersing yourself in a destination where ancient history meets vibrant modern life. From iconic landmarks to hidden local gems, Athens offers an unforgettable experience for every traveler — and at Chic Centre Suites Athens, you are perfectly positioned to explore it all.'
		=> 'Το να μένετε στην Αθήνα σημαίνει να βυθίζεστε σε έναν προορισμό όπου η αρχαία ιστορία συναντά τη σύγχρονη ζωή. Από τα εμβληματικά αξιοθέατα μέχρι τα κρυφά τοπικά στολίδια, η Αθήνα χαρίζει αξέχαστες εμπειρίες σε κάθε επισκέπτη — και από τα Chic Centre Suites, βρίσκεστε στο ιδανικό σημείο για να τα ανακαλύψετε όλα.',
	'Located just steps from Ermou Street and within a short walk of Syntagma Square and Monastiraki, your stay places you at the very center of culture, history, shopping, and gastronomy.'
		=> 'Δίπλα στην Ερμού και σε κοντινή απόσταση από το Σύνταγμα και το Μοναστηράκι, η παραμονή σας σάς τοποθετεί στο επίκεντρο του πολιτισμού, της ιστορίας, των αγορών και της γαστρονομίας.',
	'Also Worth Exploring'      => 'Αξίζει Επίσης να Εξερευνήσετε',

	/* ── Explore Athens attractions ──────────────────────────────────── */
	'Monastiraki'               => 'Μοναστηράκι',
	"Wander through the vibrant Monastiraki neighborhood, known for its bustling flea market where you can find all sorts of souvenirs, antiques, clothing, and handicrafts. Don't forget to explore the narrow streets and discover hidden gems."
		=> 'Περιηγηθείτε στη ζωντανή γειτονιά του Μοναστηρακίου, γνωστή για την πολυσύχναστη παλιά αγορά της, όπου θα βρείτε αναμνηστικά, αντίκες, ρούχα και χειροτεχνήματα. Μην παραλείψετε να εξερευνήσετε τα στενά σοκάκια και να ανακαλύψετε κρυμμένους θησαυρούς.',
	'Aerial view of Monastiraki Square with the Acropolis in the background'
		=> 'Εναέρια άποψη της Πλατείας Μοναστηρακίου με την Ακρόπολη στο βάθος',
	'Acropolis'                 => 'Ακρόπολη',
	'Start your journey at the iconic Acropolis, a UNESCO World Heritage Site and the symbol of Athens. Dominating the city skyline, this ancient citadel houses several historic monuments, including the Parthenon, Erechtheion, Propylaea, and the Temple of Athena Nike. The views of Athens from the Acropolis are breath-taking, especially during sunset.'
		=> 'Ξεκινήστε την περιήγησή σας από την εμβληματική Ακρόπολη, Μνημείο Παγκόσμιας Κληρονομιάς UNESCO και σύμβολο της Αθήνας. Αυτή η αρχαία ακρόπολη φιλοξενεί σπουδαία ιστορικά μνημεία, όπως ο Παρθενώνας, το Ερέχθειο, τα Προπύλαια και ο Ναός της Αθηνάς Νίκης. Η θέα από εκεί είναι μοναδική, ειδικά στο ηλιοβασίλεμα.',
	'The Caryatids of the Erechtheion on the Acropolis of Athens'
		=> 'Οι Καρυάτιδες του Ερεχθείου στην Ακρόπολη',
	'Acropolis Museum'          => 'Μουσείο Ακρόπολης',
	"Next to the Acropolis, visit the Acropolis Museum, which showcases an impressive collection of artifacts and sculptures from the Acropolis site. The museum provides valuable insights into the city's ancient past and the significance of the monuments you've just seen. Consistently rated as one of the best museums in the world, the Acropolis Museum was founded in 2003 to house every artifact found on the rock and on the surrounding slopes."
		=> 'Δίπλα στην Ακρόπολη, επισκεφθείτε το Μουσείο Ακρόπολης, που φιλοξενεί εντυπωσιακή συλλογή αρχαιοτήτων και γλυπτών. Αξιολογημένο ως ένα από τα καλύτερα μουσεία στον κόσμο, ιδρύθηκε το 2003 για να στεγάσει κάθε εύρημα από τον βράχο και τις πλαγιές του.',
	'Exterior of the Acropolis Museum in Athens'
		=> 'Εξωτερική άποψη του Μουσείου Ακρόπολης στην Αθήνα',
	'Syntagma Square'           => 'Πλατεία Συντάγματος',
	'Visit Syntagma Square, the central square of Athens, to witness the impressive changing of the guard ceremony in front of the Hellenic Parliament building. The square is also surrounded by shops, cafes, and is a starting point for many walking tours. Named after the Constitution that Otto, the first King of Greece, was obliged to grant in 1843, the square stands in front of the 19th-century Old Royal Palace, which has housed the Greek Parliament since 1934.'
		=> 'Επισκεφθείτε την Πλατεία Συντάγματος, το κέντρο της Αθήνας, για να παρακολουθήσετε την εντυπωσιακή αλλαγή φρουράς μπροστά στη Βουλή των Ελλήνων. Ονομάστηκε έτσι από το Σύνταγμα που αναγκάστηκε να παραχωρήσει ο Όθωνας το 1843 — η πλατεία βρίσκεται μπροστά στο Παλαιό Ανάκτορο, που στεγάζει τη Βουλή από το 1934.',
	'Syntagma Square with the Hellenic Parliament building and fountain'
		=> 'Πλατεία Συντάγματος με τη Βουλή των Ελλήνων και το σιντριβάνι',
	'Plaka'                     => 'Πλάκα',
	"After the Acropolis, explore the charming neighborhood of Plaka, located at the foot of the Acropolis hill. This old district is a maze of narrow streets lined with neoclassical buildings, traditional taverns, and souvenir shops. It's an excellent place to enjoy a leisurely stroll and get a taste of local life."
		=> 'Μετά την Ακρόπολη, εξερευνήστε τη γοητευτική Πλάκα, στους πρόποδες του βράχου. Η παλιά αυτή γειτονιά είναι ένας λαβύρινθος από στενά σοκάκια με νεοκλασικά κτίρια, παραδοσιακές ταβέρνες και καταστήματα με αναμνηστικά. Ιδανική για ατέλειωτη βόλτα και γνωριμία με την αθηναϊκή ζωή.',
	'Outdoor taverna seating along a narrow stairway street in the Plaka neighborhood'
		=> 'Τραπεζάρια σε εξωτερικό χώρο ταβέρνας σε σκαλοπάτια της Πλάκας',
	'Ancient Agora'             => 'Αρχαία Αγορά',
	"Head to the Ancient Agora, the heart of ancient Athens' political, social, and commercial life. Here, you can see well-preserved ruins like the Temple of Hephaestus, the Stoa of Attalos, and the Agora Museum, which houses artifacts from the area."
		=> 'Επισκεφθείτε την Αρχαία Αγορά, καρδιά της πολιτικής, κοινωνικής και εμπορικής ζωής της αρχαίας Αθήνας. Εκεί θα δείτε καλοδιατηρημένα ερείπια, όπως ο Ναός του Ηφαίστου, η Στοά του Αττάλου και το Μουσείο της Αγοράς.',
	'The Stoa of Attalos in the Ancient Agora with the Acropolis rising behind it'
		=> 'Η Στοά του Αττάλου στην Αρχαία Αγορά με την Ακρόπολη στο βάθος',
	'Lycabettus Hill'           => 'Λυκαβηττός',
	'For a panoramic view of Athens, hike or take a funicular ride up Lycabettus Hill. The hill offers stunning views of the city, especially during the evening when Athens lights up.'
		=> 'Για πανοραμική θέα της Αθήνας, ανεβείτε στον Λυκαβηττό με τα πόδια ή με το τελεφερίκ. Η θέα είναι εντυπωσιακή, ιδιαίτερα το βράδυ όταν η πόλη ανάβει τα φώτα της.',
	'National Archaeological Museum'    => 'Εθνικό Αρχαιολογικό Μουσείο',
	"For history enthusiasts, the National Archaeological Museum is a must-visit. It's one of the world's most extensive archaeological museums, housing a vast collection of ancient Greek artifacts, including sculptures, pottery, jewelry, and more."
		=> 'Για τους λάτρεις της ιστορίας, το Εθνικό Αρχαιολογικό Μουσείο είναι επίσκεψη υποχρεωτική. Ένα από τα πλουσιότερα αρχαιολογικά μουσεία του κόσμου, με χιλιάδες εκθέματα της αρχαίας ελληνικής τέχνης.',

	/* ── Testimonials page ───────────────────────────────────────────── */
	'Latest Reviews'          => 'Κριτικές',
	'Guest reviews'           => 'Κριτικές επισκεπτών',
	'Review from Booking.com' => 'Μεταφρασμένη κριτική Booking.com',
	'Review from Facebook'    => 'Μεταφρασμένη κριτική από Facebook',
	'Anonymous'               => 'Ανώνυμος',

	/* Date labels */
	'January 2025'  => 'Ιανουαριος 2025',
	'October 2024'  => 'Οκτωβριος 2024',
	'July 2021'     => 'Ιουλιος 2021',
	'June 2021'     => 'Ιουνιος 2021',
	'July 2020'     => 'Ιουλιος 2020',
	'December 2019' => 'Δεκεμβριος 2019',

	/* Review bodies */
	'The facilities are modern, impeccably clean, and of exceptional quality. From the bathrobes and towels to the sheets, pillows, and bed, every detail is carefully curated to ensure maximum comfort, making the stay even more restful. Our hosts were helpful and willing to assist us with anything we needed. It is truly rare to find a place that is so well-maintained and complemented by such outstanding hospitality. Additionally, the location is very secure, with three security codes required for access to the room. On the ground floor, there is a bistro, and the street outside has regular traffic, so there are always people around, which adds to the sense of safety.'
		=> 'Οι εγκαταστάσεις είναι σύγχρονες, άψογα καθαρές και εξαιρετικής ποιότητας. Από τα μπουρνούζια και τις πετσέτες μέχρι τα σεντόνια, τα μαξιλάρια και το κρεβάτι, κάθε λεπτομέρεια είναι προσεκτικά επιμελημένη για να εξασφαλίσει τη μέγιστη άνεση, κάνοντας τη διαμονή ακόμα πιο ξεκούραστη. Οι οικοδεσπότες μας βοήθησαν και ήταν πρόθυμοι να μας βοηθήσουν με ό,τι χρειαζόμασταν. Είναι πραγματικά σπάνιο να βρεις ένα μέρος που να είναι τόσο καλοδιατηρημένο και να συμπληρώνεται από τέτοια εξαιρετική φιλοξενία. Επιπλέον, η τοποθεσία είναι πολύ ασφαλής, με τρεις κωδικούς ασφαλείας που απαιτούνται για την πρόσβαση στο δωμάτιο. Στο ισόγειο, υπάρχει ένα μπιστρό, και ο δρόμος έξω έχει τακτική κυκλοφορία, έτσι υπάρχουν πάντα άνθρωποι γύρω, κάτι που προσθέτει αίσθηση ασφάλειας.',
	'Excellent location and attention and clean large room everything was excellent'
		=> 'Eξαιρετική τοποθεσία και μεγάλο δωμάτιο καθαρό.Oλα ήταν εξαιρετικά',
	'The apartment is very beautiful, furnished with great style and care, equipped with every comfort and with every kind of technology (for example: digital code to access the room, maxi TV, usb refills on the wall...). Cleaning is excellent, new dishes are present in the kitchen; also provided Nespresso coffee machine and breakfast supplies. It is located in a central area, with all kinds of services (bars, restaurants, shops). No need to take any transportation to visit the main historical sites. The owner is a welcoming and very kind person... he accompanied us for a small tour around the property. Such hospitality is really rare. We would recommend everyone to stay at one of these apartments.'
		=> 'Το διαμέρισμα ήταν πολύ όμορφο με ωραία διακόσμηση με αρκετές ανέσεις και αρκετή τεχνολογία. Για παράδειγμα υπάρχει η δυνατότητα να ανοίγει η πόρτα του διαμερίσματος με κωδικό και θύρα UCB για να φορτίζεις το κινητό. Το διαμέρισμα πεντακάθαρο, με καινούργια πιάτα κλπ., επίσης παρέχονται κάψουλες του καφέ με καφετιέρα Nespresso. Βρίσκετε σε κεντρική περιοχή με όλες τις ανέσεις σε κοντινή απόσταση (bar, εστιατόρια και καταστήματα). Δεν χεριάζετε μεταφορικό μέσο για να επισκεφθείτε τα αξιοθέατα. Ο ιδιοκτήτης ήταν πολύ φιλόξενος και πολύ ευγενικός καθώς μας εξήγησε τα πάντα. Σπάνια συναντάς τέτοια φιλοξενία. Προτείνουμε ανεπιφύλακτα τα συγκεκριμένα διαμερίσματα.',
	'Top location, the nicely furnished apartment is very well equipped, very helpful owner.'
		=> 'Καταπληκτική τοποθεσία, πολύ ωραία διακόσμηση, πλήρως εξοπλισμένο διαμέρισμα και πολύ βοηθητικός ο ιδιοκτήτης!',
	'Great location for a short stay in the city. Everything is thought out to the smallest detail. Highest mark!'
		=> 'Τέλεια τοποθεσία για την διαμονή μας στην Αθήνα. Όλα είναι προσεγμένα με την κάθε λεπτομέρεια. 10/10',
	'Just one word: GREAT!!! Clean facility and in the center of Athens nothing is missing. Jorgos number one, welcomed us warmly, gave us lots of advice of all kinds.. making us do a quick and short city tour.'
		=> 'Μόνο μια λέξη: Καταπληκτικό!!! Καθαρό και στο κέντρο της Αθήνας, τίποτα λιγότερο. Ο Γιώργος καταπληκτικός, ζεστή υποδοχή με πολλές συμβουλές για το ταξίδι μας.. επίσης μας έκανε μια σύντομη ξενάγηση στην πόλη!',
	'The room itself was very clean and tidy, great bed and bathroom! You also get a lot of privacy. Owners very accommodating, extended our stay for some hours after our request for late check out.'
		=> 'Το δωμάτιο πολύ καθαρό και συγυρισμένο, καταπληκτικό κρεβάτι και μπάνιο! Επίσης έχει αρκετή ιδιωτικότητα, επεκτείναμε την κράτηση μας αφού ζητήσαμε αργοπορημένη αναχώρηση και έγινε αποδεκτό!',
	'For starters let me just say that the owner was super friendly and helpful. Unbelievable clean, everything was new and spotless. Even though the apartment is located Centre of all attractions where it\'s super crowded, we couldn\'t hear anything. Located-as mentioned- in the center of everything, in a walking distance. We loved Chic Centre Suites and we have already bookmarked it on our favorites.'
		=> 'Για αρχή να σας πω ότι ο ιδιοκτήτης ήταν πολύ φιλικός και βοηθητικός. Απίστευτα καθαρό και ολοκαίνουργιο διαμέρισμα! Το διαμέρισμα βρίσκετε στο κέντρο και όλα τα αξιοθέατα είναι πολύ κοντά. Όπως σας ανάφερα βρίσκετε στην καρδιά της Αθήνας και πας παντού με τα πόδια! Αγαπήσαμε τα Chic Centre Suites και είναι ήδη στην λίστα των αγαπημένων μας!',
	'The Location was amazing in the heart of center and shops. rooms were very clean and new. Comfortable room excellent bed.'
		=> 'Η τοποθεσία ήταν καταπληκτική στην καρδιά του κέντρου και των καταστημάτων. τα δωμάτια ήταν πολύ καθαρά και νέα. Άνετο δωμάτιο εξαιρετικό κρεβάτι.',
	'Amenity is very good ・There are many outlets ・Comfortable ・Rooms are very clean ・Close to the center ・Equipped with excellent facilities It was a stay with people, but he helped me in various ways. If I have a chance to visit Athens again, I would definitely like to use it again.'
		=> 'Πολύ καλές ανέσεις ・ Υπάρχουν πολλά καταστήματα ・ Άνετα ・ Τα δωμάτια είναι πολύ καθαρά ・ Κοντά στο κέντρο ・ Εξοπλισμένα με εξαιρετικές εγκαταστάσεις. Εάν έχω την ευκαιρία να επισκεφθώ ξανά την Αθήνα, σίγουρα θα ήθελα να τα χρησιμοποιήσω ξανά.',
	'Quality of services and very central location that allows you to enjoy Athens. The reception is very attentive and listens to our requests.'
		=> 'Ποιότητα υπηρεσιών και πολύ κεντρική τοποθεσία που σας επιτρέπει να απολαύσετε την Αθήνα. Η ρεσεψιόν είναι πολύ προσεκτική και ακούει τα αιτήματά μας.',
	'Excellent location, apartment fully equipped and everything brand new.Pleasant surprise the private balcony area in which smokers can use.Espresso machine and coffee tablets available as well as jacuzzi.Strongly recommended.'
		=> 'Εξαιρετική τοποθεσία, διαμέρισμα πλήρως εξοπλισμένο και όλα ολοκαίνουργια. Ευχάριστη έκπληξη το ιδιωτικό μπαλκόνι το οποίο μπορούν να χρησιμοποιούν οι καπνιστές. Διατίθεται μηχανή εσπρέσο και δισκία καφέ καθώς και τζακούζι. Συνιστάται ανεπιφύλακτα.',
	'Came to Athens many times, stayed in several places. This apartment were the best place so far. Just 1min walking distance from Ermou. The apartment is such a luxury with good facilities. The room was very clean and the Wifi signal was good too. Mattress was unbelievable ! So comfortable . Staff was very friendly . Value for money ! Looking forward to visit your apartment in March too!!!'
		=> 'Ήρθα πολλές φορές στην Αθήνα, έμεινα σε πολλά μέρη. Αυτό το διαμέρισμα ήταν το καλύτερο μέρος μέχρι τώρα. Μόλις 1 λεπτό με τα πόδια από την Ερμού. Το διαμέρισμα είναι τόσο πολυτελές με καλές εγκαταστάσεις. Το δωμάτιο ήταν πολύ καθαρό και το σήμα Wifi ήταν επίσης καλό. Το στρώμα ήταν απίστευτο! Τόσο άνετο. Το προσωπικό ήταν πολύ φιλικό. Σχέση ποιότητας και τιμής ! Ανυπομονώ να επισκεφθώ το διαμέρισμά σας τον Μάρτιο !!!',
	'Strongly recommended for a luxury and enjoyable stay in Athens. Perfect location in the city centre and fully equipped to accommodate family or couples. Ideal for business trips as well. Wise choice as you enjoy the facilities of a 5 star hotel with a very reasonable and attractive price. Looking forward to repeat our trip to Athens soon and stay at Chic Centre Suites again.'
		=> 'Συνιστάται σταθερά για μια πολυτελή και ευχάριστη διαμονή στην Αθήνα. Ιδανική τοποθεσία στο κέντρο της πόλης και πλήρως εξοπλισμένα για να φιλοξενήσουν οικογένεια ή ζευγάρια. Ιδανικό και για επαγγελματικά ταξίδια. Σοφή επιλογή καθώς απολαμβάνετε τις εγκαταστάσεις ενός ξενοδοχείου 5 αστέρων με πολύ λογική και ελκυστική τιμή. Ανυπομονούμε να επαναλάβουμε το ταξίδι μας στην Αθήνα σύντομα και να μείνουμε ξανά στο Chic Center Suites.',

	/* ── Suite descriptions: Chavriou 2 ──────────────────────────────── */
	'Suite 1 is a modern studio in Athens city center, ideally located just steps from Ermou Street, one of the most popular shopping and pedestrian areas in the capital. This 30m² Athens accommodation is perfect for couples or solo travelers seeking a central and comfortable stay.'
		=> 'Η Σουίτα 1 είναι ένα σύγχρονο studio στο κέντρο της Αθήνας, ιδανικά τοποθετημένο δίπλα στην Ερμού, έναν από τους πιο δημοφιλείς πεζόδρομους της πρωτεύουσας. Το 30τ.μ. αυτό κατάλυμα είναι ιδανικό για ζευγάρια ή μεμονωμένους ταξιδιώτες που αναζητούν κεντρική και άνετη παραμονή.',
	'The studio features a comfortable double bed, a cozy seating area with sofa, and a private balcony, offering the perfect space to relax after a day of exploring Athens. The interior combines natural tones with a modern and functional layout, creating a warm and practical environment.'
		=> 'Το studio διαθέτει διπλό κρεβάτι, άνετο χώρο καθιστικού με καναπέ και ιδιωτικό μπαλκόνι — ιδανικό για χαλάρωση μετά από μια μέρα εξερεύνησης στην Αθήνα. Το εσωτερικό συνδυάζει φυσικές αποχρώσεις με σύγχρονη διάταξη.',
	'Guests can enjoy a fully equipped kitchen, a modern bathroom with shower, smart TV access, high-speed WiFi and all essential amenities for a convenient stay in Athens.'
		=> 'Οι επισκέπτες απολαμβάνουν πλήρως εξοπλισμένη κουζίνα, σύγχρονο μπάνιο με ντους, Smart TV, γρήγορο Wi-Fi και όλες τις απαραίτητες παροχές για άνετη παραμονή στην Αθήνα.',
	'Located within walking distance of major attractions, restaurants, cafés, and shopping streets, Suite 1 is an excellent choice for travelers looking for a centrally located studio near Ermou Street and Syntagma Square with a private balcony and easy city access.'
		=> 'Σε κοντινή απόσταση από αξιοθέατα, εστιατόρια, καφέ και εμπορικούς δρόμους, η Σουίτα 1 είναι εξαιρετική επιλογή για ταξιδιώτες που αναζητούν κεντρικό studio κοντά στην Ερμού και το Σύνταγμα με ιδιωτικό μπαλκόνι.',

	'Suite 2 is a spacious one-bedroom apartment in Athens city center, ideally located in the heart of the capital. This 41m² Athens accommodation is perfect for families or small groups of up to 4 guests seeking extra comfort, space, and convenience in a central location.'
		=> 'Η Σουίτα 2 είναι ένα ευρύχωρο διαμέρισμα ενός υπνοδωματίου στο κέντρο της Αθήνας. Το 41τ.μ. αυτό κατάλυμα είναι ιδανικό για οικογένειες ή μικρές παρέες έως 4 ατόμων που αναζητούν άνεση και χώρο σε κεντρική τοποθεσία.',
	'The apartment features a separate bedroom with a comfortable double bed and a living room with a sofa bed, offering privacy and flexibility for both short and longer stays. Its functional layout makes it an excellent choice for families visiting Athens.'
		=> 'Το διαμέρισμα διαθέτει ξεχωριστό υπνοδωμάτιο με διπλό κρεβάτι και καθιστικό με καναπέ-κρεβάτι, προσφέροντας ιδιωτικότητα και ευελιξία. Η λειτουργική διαρρύθμισή του το καθιστά ιδανικό για οικογένειες.',
	'Guests can enjoy a fully equipped kitchen, a modern bathroom with shower, smart TV access, high-speed WiFi and the added convenience of a washing machine, ideal for extended stays. A private balcony provides a relaxing outdoor space after a day exploring the city.'
		=> 'Πλήρως εξοπλισμένη κουζίνα, σύγχρονο μπάνιο, Smart TV, γρήγορο Wi-Fi και πλυντήριο ρούχων — ιδανικό για παρατεταμένες διαμονές. Το ιδιωτικό μπαλκόνι προσφέρει υπαίθριο χώρο χαλάρωσης.',
	"Located within walking distance of Athens' main attractions, restaurants, cafés, and shopping areas, Suite 2 is a practical and comfortable base for travelers looking for a centrally located family apartment in Athens with extra space and home-like amenities."
		=> 'Σε βολική απόσταση από τα κύρια αξιοθέατα, εστιατόρια, καφέ και εμπορικά κέντρα, η Σουίτα 2 είναι πρακτική και άνετη βάση για οικογένειες στο κέντρο της Αθήνας.',

	'Suite 3 is a comfortable studio in Athens city center, located in the heart of the capital. This 23m² Athens accommodation is ideal for couples or solo travelers seeking a practical and well-located stay in a central area of the city.'
		=> 'Η Σουίτα 3 είναι ένα άνετο studio στο κέντρο της Αθήνας. Το 23τ.μ. κατάλυμα είναι ιδανικό για ζευγάρια ή μεμονωμένους ταξιδιώτες που αναζητούν πρακτική και καλά τοποθετημένη παραμονή.',
	'The studio features a comfortable double bed, a small seating area with sofa, and a functional layout designed for easy and convenient living during your stay in Athens. Natural tones with olive accents create a warm and relaxing atmosphere, inspired by Mediterranean aesthetics.'
		=> 'Το studio διαθέτει διπλό κρεβάτι, μικρό καθιστικό με καναπέ και λειτουργική διαρρύθμιση. Φυσικές αποχρώσεις με ελιά δημιουργούν ζεστή ατμόσφαιρα εμπνευσμένη από τη Μεσόγειο.',
	"Located within walking distance of Athens' main attractions, restaurants, cafés, and shopping streets, Suite 3 is an excellent choice for travelers looking for a centrally located and comfortable studio in Athens with easy access to everything the city has to offer."
		=> 'Σε κοντινή απόσταση από αξιοθέατα, εστιατόρια, καφέ και εμπορικούς δρόμους, η Σουίτα 3 είναι εξαιρετική επιλογή για άνετο κεντρικό studio στην Αθήνα.',

	'Suite 4 is a modern studio in Athens city center, located in the heart of the capital. This 24m² Athens accommodation is ideal for couples or solo travelers seeking a comfortable and well-located stay with easy access to the city\'s main highlights.'
		=> 'Η Σουίτα 4 είναι ένα σύγχρονο studio στο κέντρο της Αθήνας. Το 24τ.μ. κατάλυμα είναι ιδανικό για ζευγάρια ή μεμονωμένους ταξιδιώτες με εύκολη πρόσβαση στα κύρια αξιοθέατα.',
	'The studio features a comfortable double bed, a small seating area with sofa, and a private balcony, offering the perfect space to enjoy your morning coffee or relax after a day of sightseeing in Athens. The layout is designed for comfort and convenience in a compact urban space.'
		=> 'Διπλό κρεβάτι, καθιστικό με καναπέ και ιδιωτικό μπαλκόνι για τον πρωινό καφέ ή για χαλάρωση μετά την εξερεύνηση. Η διαρρύθμιση προσφέρει άνεση σε compact αστικό χώρο.',
	'Guests can enjoy a fully equipped kitchen, a modern bathroom with shower, smart TV access, high-speed WiFi and all essential amenities for a pleasant stay in Athens.'
		=> 'Πλήρως εξοπλισμένη κουζίνα, σύγχρονο μπάνιο με ντους, Smart TV, γρήγορο Wi-Fi και όλες οι απαραίτητες παροχές για ευχάριστη παραμονή στην Αθήνα.',
	'Located within walking distance of major attractions, restaurants, cafés, and shopping areas, Suite 4 is an excellent choice for travelers looking for a centrally located studio in Athens with a private balcony and modern comforts.'
		=> 'Κοντά σε αξιοθέατα, εστιατόρια, καφέ και εμπορικές περιοχές, η Σουίτα 4 είναι εξαιρετική επιλογή για κεντρικό studio με ιδιωτικό μπαλκόνι και σύγχρονες ανέσεις.',

	'Suite 5 is a comfortable studio in Athens city center, located in the heart of the capital. This 24m² Athens accommodation is ideal for couples or solo travelers seeking a relaxed, practical, and centrally located stay.'
		=> 'Η Σουίτα 5 είναι ένα άνετο studio στο κέντρο της Αθήνας. Το 24τ.μ. κατάλυμα είναι ιδανικό για ζευγάρια ή μεμονωμένους ταξιδιώτες που αναζητούν χαλαρή, πρακτική και κεντρική παραμονή.',
	'The studio features a comfortable double bed, a seating area with sofa, and a private balcony, offering a quiet outdoor space to unwind after a busy day exploring Athens. The functional layout ensures a convenient and comfortable living experience in a compact city setting.'
		=> 'Διπλό κρεβάτι, καθιστικό με καναπέ και ιδιωτικό μπαλκόνι για ήσυχη χαλάρωση μετά από πολυάσχολη μέρα στην πόλη. Λειτουργική διαρρύθμιση για άνετη διαμονή σε compact αστικό χώρο.',
	'Guests can enjoy a fully equipped kitchen, a modern bathroom with shower, smart TV access, high-speed WiFi and all essential amenities for both short and extended stays in Athens.'
		=> 'Πλήρως εξοπλισμένη κουζίνα, σύγχρονο μπάνιο, Smart TV, γρήγορο Wi-Fi και όλες οι απαραίτητες παροχές για σύντομες ή παρατεταμένες παραμονές.',
	'Located within walking distance of major attractions, restaurants, cafés, and shopping streets, Suite 5 is an excellent choice for travelers looking for a centrally located studio in Athens with a balcony and easy access to everything the city offers.'
		=> 'Κοντά σε αξιοθέατα, εστιατόρια, καφέ και εμπορικές περιοχές, η Σουίτα 5 είναι εξαιρετική επιλογή για κεντρικό studio με μπαλκόνι.',

	'Suite 6 is a studio with balcony in Athens city center, located in the heart of the capital. This 26m² Athens accommodation is ideal for couples or solo travelers seeking a comfortable, well-balanced, and centrally located stay.'
		=> 'Η Σουίτα 6 είναι ένα studio με μπαλκόνι στο κέντρο της Αθήνας. Το 26τ.μ. κατάλυμα είναι ιδανικό για ζευγάρια ή μεμονωμένους ταξιδιώτες που αναζητούν άνετη και κεντρική παραμονή.',
	'The studio features a comfortable double bed, a seating area with sofa, and a private balcony, offering a pleasant outdoor space to relax after a day exploring Athens. With a slightly more spacious layout than standard studios, it provides added comfort for both short and longer stays.'
		=> 'Διπλό κρεβάτι, καθιστικό με καναπέ και ιδιωτικό μπαλκόνι για ευχάριστη ανάπαυλα μετά την εξερεύνηση της πόλης. Λίγο πιο ευρύχωρο από τα τυπικά studios, προσφέρει επιπλέον άνεση.',
	'Located within walking distance of major attractions, restaurants, cafés, and shopping streets, Suite 6 is an excellent choice for travelers looking for a centrally located studio in Athens with a balcony, extra space, and modern comfort.'
		=> 'Κοντά σε αξιοθέατα, εστιατόρια, καφέ και εμπορικές περιοχές, η Σουίτα 6 είναι εξαιρετική επιλογή για κεντρικό studio με μπαλκόνι, επιπλέον χώρο και σύγχρονες ανέσεις.',

	/* ── Suite descriptions: Thiseos 11 ──────────────────────────────── */
	'Avra Suite is a spacious 52m² one-bedroom apartment in Athens city center, designed for families or small groups of up to four guests. Located near the city\'s most iconic landmarks, it offers easy access to the Acropolis, Monastiraki, and Ermou Street.'
		=> 'Η Σουίτα Avra είναι ένα ευρύχωρο διαμέρισμα 52τ.μ. ενός υπνοδωματίου στο κέντρο της Αθήνας, ιδανικό για οικογένειες ή μικρές παρέες έως τέσσερα άτομα. Κοντά στα πιο εμβληματικά αξιοθέατα της πόλης, με εύκολη πρόσβαση στην Ακρόπολη, το Μοναστηράκι και την Ερμού.',
	'The suite features a private bedroom, a separate living area with sofa bed, modern bathroom with Jacuzzi bathtub for added comfort after a day of sightseeing, a private balcony, fully equipped kitchen, high-speed WiFi, smart TV access and modern entertainment amenities make it ideal for both short city breaks and longer stays.'
		=> 'Ξεχωριστό υπνοδωμάτιο, καθιστικό με καναπέ-κρεβάτι, σύγχρονο μπάνιο με τζακούζι, ιδιωτικό μπαλκόνι, πλήρως εξοπλισμένη κουζίνα, γρήγορο Wi-Fi και Smart TV — ιδανικό τόσο για σύντομες αστικές διαδρομές όσο και για παρατεταμένες παραμονές.',
	'With generous space and premium features, Avra Suite is a perfect choice for travelers seeking comfort in the heart of Athens.'
		=> 'Με άφθονο χώρο και premium παροχές, η Σουίτα Avra είναι η ιδανική επιλογή για ταξιδιώτες που αναζητούν άνεση στην καρδιά της Αθήνας.',

	'Zakynthos Suite is a stylish studio apartment in Athens city center, designed for couples seeking a relaxing and convenient stay close to the historical center.'
		=> 'Η Σουίτα Zakynthos είναι ένα κομψό studio στο κέντρο της Αθήνας, σχεδιασμένο για ζευγάρια που αναζητούν χαλαρή και βολική παραμονή κοντά στην Ιστορική Τριγωνιά.',
	'This elegant 28m² suite features a comfortable double bed, a cozy seating area, and a private balcony where guests can enjoy their morning coffee or unwind after exploring the city. Island-inspired details create a warm and welcoming atmosphere, ideal for short stays or city breaks.'
		=> 'Το κομψό 28τ.μ. studio διαθέτει διπλό κρεβάτι, χώρο καθιστικού και ιδιωτικό μπαλκόνι για τον πρωινό καφέ ή χαλάρωση μετά την εξερεύνηση. Νησιώτικες λεπτομέρειες δημιουργούν ζεστή ατμόσφαιρα, ιδανική για σύντομες αποδράσεις.',
	'Guests can also enjoy a fully equipped kitchen, a modern bathroom with shower, high-speed WiFi, and smart TV access, ensuring both comfort and practicality throughout their stay.'
		=> 'Πλήρως εξοπλισμένη κουζίνα, σύγχρονο μπάνιο με ντους, γρήγορο Wi-Fi και Smart TV εξασφαλίζουν άνεση και πρακτικότητα σε όλη τη διάρκεια της παραμονής.',
	"Located near Athens' most iconic attractions, shopping streets, and local dining spots, Zakynthos Suite offers the perfect balance of style, convenience, and central location."
		=> 'Κοντά στα πιο εμβληματικά αξιοθέατα, εμπορικές περιοχές και τοπικές γαστρονομικές εμπειρίες, η Σουίτα Zakynthos προσφέρει τέλεια ισορροπία στυλ, βολικότητας και κεντρικής τοποθεσίας.',

	'Santorini Suite is a stylish studio apartment in Athens historical center, ideal for couples seeking comfort and convenience in a prime location.'
		=> 'Η Σουίτα Santorini είναι ένα κομψό studio στην Ιστορική Τριγωνιά της Αθήνας, ιδανικό για ζευγάρια που αναζητούν άνεση και βολικότητα σε εξαιρετική τοποθεσία.',
	'This thoughtfully island-designed suite includes a cozy double bed, seating area, private balcony, a Cycladic bathroom style with shower and fully equipped kitchen. Guests can enjoy modern amenities such as smart TV access and high-speed WiFi, ensuring a seamless stay.'
		=> 'Αυτό το νησιωτικό studio διαθέτει άνετο διπλό κρεβάτι, χώρο καθιστικού, ιδιωτικό μπαλκόνι, Κυκλαδικό μπάνιο με ντους και πλήρως εξοπλισμένη κουζίνα. Smart TV και γρήγορο Wi-Fi εξασφαλίζουν απρόσκοπτη παραμονή.',
	"Within walking distance of Athens' major attractions, this studio offers the perfect base for discovering the city while enjoying a boutique-style atmosphere."
		=> 'Σε κοντινή απόσταση από τα κύρια αξιοθέατα της Αθήνας, αυτό το studio προσφέρει ιδανική βάση για την εξερεύνηση της πόλης με boutique ατμόσφαιρα.',

	'Kohili Suite is a spacious premium apartment in Athens city center, designed for travelers who value comfort, style, and relaxation in a central location.'
		=> 'Η Σουίτα Kohili είναι ένα ευρύχωρο premium διαμέρισμα στο κέντρο της Αθήνας, σχεδιασμένο για ταξιδιώτες που εκτιμούν την άνεση, το στυλ και τη χαλάρωση.',
	'This elegant 37m² suite features a comfortable double bed, a cozy seating area, and a private balcony, creating the perfect setting for a memorable city stay. Earthy tones and soft lighting enhance the calming atmosphere, while the private Jacuzzi bathtub offers an added touch of luxury after a day exploring Athens.'
		=> 'Το κομψό 37τ.μ. suite διαθέτει διπλό κρεβάτι, χώρο καθιστικού και ιδιωτικό μπαλκόνι — ιδανικό για αξέχαστη αστική παραμονή. Γήινες αποχρώσεις και μαλακός φωτισμός δημιουργούν ηρεμιστική ατμόσφαιρα, ενώ το ιδιωτικό τζακούζι προσθέτει πινελιά πολυτέλειας.',
	'Ideal for couples, the suite also includes a fully equipped kitchen, modern bathroom with Jacuzzi bathtub, high-speed WiFi, and smart TV access, ensuring a seamless and enjoyable experience.'
		=> 'Ιδανικό για ζευγάρια, το suite διαθέτει επίσης πλήρως εξοπλισμένη κουζίνα, σύγχρονο μπάνιο με τζακούζι, γρήγορο Wi-Fi και Smart TV για απρόσκοπτη και ευχάριστη εμπειρία.',
	'Located close to Athens\' historical landmarks, shopping areas, and vibrant dining spots, Kohili Suite combines premium comfort with the convenience of staying in the heart of the city.'
		=> 'Κοντά στα ιστορικά μνημεία, εμπορικές περιοχές και ζωντανές γαστρονομικές επιλογές, η Σουίτα Kohili συνδυάζει premium άνεση με την ευκολία της κεντρικής τοποθεσίας.',

	'Korali Suite is a stylish studio apartment in Athens city center, ideal for couples seeking a comfortable and well-located stay in the heart of the capital.'
		=> 'Η Σουίτα Korali είναι ένα κομψό studio στο κέντρο της Αθήνας, ιδανικό για ζευγάρια που αναζητούν άνετη και καλά τοποθετημένη παραμονή.',
	'This inviting 20m² suite features a cozy double bed, a small seating area with sofa, and warm interiors inspired by soft coral tones, creating a relaxing atmosphere after a day of exploring Athens.'
		=> 'Αυτό το ζεστό 20τ.μ. suite διαθέτει άνετο διπλό κρεβάτι, μικρό καθιστικό με καναπέ και εσωτερικά εμπνευσμένα από απαλές κοραλί αποχρώσεις, δημιουργώντας χαλαρωτική ατμόσφαιρα.',
	'Guests can enjoy a fully equipped kitchen, a modern bathroom with shower, high-speed WiFi, and smart TV access, ensuring both comfort and convenience throughout their stay.'
		=> 'Πλήρως εξοπλισμένη κουζίνα, σύγχρονο μπάνιο με ντους, γρήγορο Wi-Fi και Smart TV εξασφαλίζουν άνεση και βολικότητα.',
	'Within walking distance of Athens\' main attractions, shopping streets, and local restaurants, Korali Suite offers a practical and welcoming base for short stays, city breaks, and holiday escapes.'
		=> 'Σε κοντινή απόσταση από τα κύρια αξιοθέατα, εμπορικούς δρόμους και τοπικά εστιατόρια, η Σουίτα Korali προσφέρει πρακτική και φιλόξενη βάση για σύντομες παραμονές και αστικές αποδράσεις.',

	'Mykonos Suite is a spacious family suite in Athens city center, ideal for families, couples, or small groups of up to 4 guests seeking comfort and convenience in the heart of the capital.'
		=> 'Η Σουίτα Mykonos είναι ένα ευρύχωρο οικογενειακό suite στο κέντρο της Αθήνας, ιδανικό για οικογένειες, ζευγάρια ή μικρές παρέες έως 4 ατόμων.',
	'This 35m² suite features a comfortable double bed and a living room with a sofa bed, offering flexible accommodation for families or groups. A large private terrace provides the perfect outdoor space to relax and unwind after a day exploring Athens.'
		=> 'Το 35τ.μ. suite διαθέτει διπλό κρεβάτι και καθιστικό με καναπέ-κρεβάτι για ευέλικτες διατάξεις. Η μεγάλη ιδιωτική βεράντα προσφέρει ιδανικό υπαίθριο χώρο για χαλάρωση.',
	'Designed with subtle island-inspired elements, the suite combines a bright and relaxing atmosphere with modern functionality. Guests can enjoy a fully equipped kitchen, a contemporary Mykonian-style bathroom with shower, smart TV access, high-speed WiFi, and all essential amenities for a comfortable stay.'
		=> 'Με Μυκονιάτικες νησιωτικές πινελιές, συνδυάζει φωτεινή και χαλαρωτική ατμόσφαιρα με σύγχρονη λειτουργικότητα. Πλήρως εξοπλισμένη κουζίνα, σύγχρονο μπάνιο Μυκονιάτικου στυλ, Smart TV και γρήγορο Wi-Fi.',
	"Located within walking distance of Athens' main attractions, shopping areas, restaurants, and cultural sites, Mykonos Suite is an excellent choice for travelers looking for a centrally located apartment in Athens with extra space and a private terrace."
		=> 'Κοντά στα κύρια αξιοθέατα, εμπορικές περιοχές, εστιατόρια και πολιτιστικούς χώρους, η Σουίτα Mykonos είναι εξαιρετική επιλογή για κεντρικό διαμέρισμα στην Αθήνα με επιπλέον χώρο και ιδιωτική βεράντα.',

	'Paros Suite is a spacious family suite in Athens city center, ideal for families or small groups of up to 4 guests looking for a comfortable and well-located stay in the heart of the capital.'
		=> 'Η Σουίτα Paros είναι ένα ευρύχωρο οικογενειακό suite στο κέντρο της Αθήνας, ιδανικό για οικογένειες ή μικρές παρέες έως 4 ατόμων.',
	'This bright 36m² suite features a separate bedroom with a double bed and a living area with a sofa bed, offering flexible sleeping arrangements for families or groups. Inspired by Cycladic design, the interiors combine minimal aesthetics, natural light, and a welcoming atmosphere.'
		=> 'Το φωτεινό 36τ.μ. suite διαθέτει ξεχωριστό υπνοδωμάτιο με διπλό κρεβάτι και καθιστικό με καναπέ-κρεβάτι. Κυκλαδικής έμπνευσης εσωτερικά με μινιμαλιστική αισθητική, φυσικό φως και φιλόξενη ατμόσφαιρα.',
	'Guests can enjoy a fully equipped kitchen, a Cycladic bathroom style, smart TV access, high-speed WiFi, ensuring both comfort and convenience throughout their stay in Athens.'
		=> 'Πλήρως εξοπλισμένη κουζίνα, μπάνιο Κυκλαδικού στυλ, Smart TV και γρήγορο Wi-Fi εξασφαλίζουν άνεση και βολικότητα σε όλη τη διάρκεια της παραμονής.',
	'Located in the historic center of Athens, within walking distance of major attractions, restaurants, shops, and cultural sites, Paros Suite offers a practical and stylish base for city breaks, family holidays, and short stays in Athens.'
		=> 'Στην Ιστορική Τριγωνιά της Αθήνας, κοντά σε αξιοθέατα, εστιατόρια, καταστήματα και πολιτιστικούς χώρους, η Σουίτα Paros είναι πρακτική και κομψή βάση για αστικές αποδράσεις και οικογενειακές διακοπές.',

	'Ammos Suite is a bright and functional studio in Athens city center, ideally located in the historic center of the capital. Perfect for couples, this 19m² Athens accommodation offers a comfortable and well-connected base for exploring the city.'
		=> 'Η Σουίτα Ammos είναι ένα φωτεινό και λειτουργικό studio στην Ιστορική Τριγωνιά της Αθήνας. Ιδανική για ζευγάρια, αυτή η 19τ.μ. παραμονή προσφέρει άνετη και εύκολα προσβάσιμη βάση για την εξερεύνηση της πόλης.',
	'The studio features a cozy double bed, a small seating area with sofa, and a private terrace, ideal for enjoying your morning coffee or relaxing after a day of sightseeing in Athens. Warm sandy tones throughout the interior create a calm and inviting atmosphere inspired by natural island aesthetics.'
		=> 'Άνετο διπλό κρεβάτι, μικρό καθιστικό με καναπέ και ιδιωτική βεράντα για τον πρωινό καφέ ή χαλάρωση μετά την εξερεύνηση. Ζεστές αμμουδερές αποχρώσεις δημιουργούν ήρεμη, νησιώτικης έμπνευσης ατμόσφαιρα.',
	'Guests can enjoy a fully equipped kitchen, a modern bathroom with shower, smart TV access, high-speed WiFi, and all essential amenities for a comfortable short or extended stay in Athens.'
		=> 'Πλήρως εξοπλισμένη κουζίνα, σύγχρονο μπάνιο με ντους, Smart TV, γρήγορο Wi-Fi και όλες οι απαραίτητες παροχές για σύντομη ή παρατεταμένη παραμονή.',
	"Located within walking distance of major attractions, restaurants, cafés, and shopping areas, Ammos Suite is an excellent choice for travelers looking for a central studio in Athens with a private terrace and easy access to the city's highlights."
		=> 'Κοντά σε αξιοθέατα, εστιατόρια, καφέ και εμπορικές περιοχές, η Σουίτα Ammos είναι εξαιρετική επιλογή για κεντρικό studio με ιδιωτική βεράντα.',

	'Ermou Suite is a modern studio in Athens city center, ideally located just steps away from Ermou Street, one of the most popular shopping streets in Athens. Perfect for couples or solo travelers seeking a central and convenient stay in the heart of the capital.'
		=> 'Η Σουίτα Ermou είναι ένα σύγχρονο studio δίπλα στην Ερμού, τον πιο δημοφιλή εμπορικό δρόμο της Αθήνας. Ιδανική για ζευγάρια ή μεμονωμένους ταξιδιώτες που αναζητούν κεντρική και βολική παραμονή.',
	'This functional 19m² studio features a comfortable double bed, a cozy seating area, and a private terrace, offering a relaxing outdoor space after a day of exploring Athens. The contemporary design creates a clean and comfortable environment for both short and extended stays.'
		=> 'Το λειτουργικό 19τ.μ. studio διαθέτει διπλό κρεβάτι, χώρο καθιστικού και ιδιωτική βεράντα για υπαίθρια χαλάρωση. Σύγχρονη σχεδίαση για καθαρό και άνετο περιβάλλον.',
	'Guests can enjoy a fully equipped kitchen, a modern bathroom, smart TV access, high-speed WiFi, and essential amenities for a comfortable stay. The studio is designed to provide convenience, privacy, and functionality in a compact urban space.'
		=> 'Πλήρως εξοπλισμένη κουζίνα, σύγχρονο μπάνιο, Smart TV, γρήγορο Wi-Fi και βασικές παροχές. Σχεδιασμένο για βολικότητα, ιδιωτικότητα και λειτουργικότητα σε compact αστικό χώρο.',
	"Located within walking distance of Athens' main attractions, restaurants, cafés, and shopping areas, Ermou Suite is an excellent choice for travelers looking for a centrally located apartment near Ermou Street and Syntagma Square."
		=> 'Κοντά στα κύρια αξιοθέατα, εστιατόρια, καφέ και εμπορικές περιοχές, η Σουίτα Ermou είναι εξαιρετική επιλογή για κεντρική παραμονή κοντά στην Ερμού και το Σύνταγμα.',

	'Pelagos Suite is a stylish studio in Athens city center, ideally located in the historic center of the capital. Perfect for couples or solo travelers, this 27m² Athens accommodation offers comfort, convenience, and a central base for exploring the city.'
		=> 'Η Σουίτα Pelagos είναι ένα κομψό studio στην Ιστορική Τριγωνιά της Αθήνας. Ιδανική για ζευγάρια ή μεμονωμένους ταξιδιώτες, αυτή η 27τ.μ. παραμονή προσφέρει άνεση, βολικότητα και κεντρική βάση.',
	'The studio features a comfortable double bed, a cozy seating area with sofa, and a private terrace, ideal for enjoying your morning coffee or relaxing after a day of sightseeing in Athens. Soft blue tones inspired by the Aegean Sea create a calming and refreshing atmosphere throughout the space.'
		=> 'Διπλό κρεβάτι, χώρος καθιστικού με καναπέ και ιδιωτική βεράντα για τον πρωινό καφέ ή χαλάρωση. Απαλές μπλε αποχρώσεις εμπνευσμένες από το Αιγαίο δημιουργούν ηρεμιστική ατμόσφαιρα.',
	'Guests can enjoy a fully equipped kitchen, a modern bathroom with shower, smart TV access, high-speed WiFi, and all essential amenities for a comfortable stay in Athens.'
		=> 'Πλήρως εξοπλισμένη κουζίνα, σύγχρονο μπάνιο με ντους, Smart TV, γρήγορο Wi-Fi και όλες οι απαραίτητες παροχές για άνετη παραμονή.',
	'Located within walking distance of major attractions, restaurants, cafés, and shopping streets, Pelagos Suite is an excellent choice for travelers seeking a central studio in Athens with a private terrace and relaxing island-inspired design.'
		=> 'Κοντά σε αξιοθέατα, εστιατόρια, καφέ και εμπορικές περιοχές, η Σουίτα Pelagos είναι εξαιρετική επιλογή για κεντρικό studio με ιδιωτική βεράντα και νησιώτικης έμπνευσης σχεδίαση.',

	/* ── Suite descriptions: Thiseos 13 ──────────────────────────────── */
	'Ocean Suite is a modern studio in Athens city center, located in the historic heart of the capital. Ideal for couples or solo travelers, this 30m² Athens accommodation offers a comfortable, stylish, and well-connected base for exploring the city.'
		=> 'Η Σουίτα Ocean είναι ένα σύγχρονο studio στην καρδιά της Αθήνας. Ιδανική για ζευγάρια ή μεμονωμένους ταξιδιώτες, αυτή η 30τ.μ. παραμονή προσφέρει άνετη, κομψή βάση για την εξερεύνηση της πόλης.',
	'The studio features a comfortable double bed and a cozy seating area with sofa, designed in a clean and modern layout. Calming blue tones inspired by the sea create a fresh and relaxing atmosphere, perfect for unwinding after a day in Athens.'
		=> 'Διπλό κρεβάτι και χώρος καθιστικού με καναπέ, με καθαρή, σύγχρονη διαρρύθμιση. Ήρεμες μπλε αποχρώσεις εμπνευσμένες από τη θάλασσα δημιουργούν αναζωογονητική ατμόσφαιρα.',
	'Guests can enjoy a fully equipped kitchen, a modern bathroom with shower, smart TV access, high-speed WiFi, and all essential amenities needed for a convenient short or extended stay.'
		=> 'Πλήρως εξοπλισμένη κουζίνα, σύγχρονο μπάνιο με ντους, Smart TV, γρήγορο Wi-Fi και όλες οι απαραίτητες παροχές για σύντομη ή παρατεταμένη παραμονή.',
	"Located within walking distance of Athens' main attractions, restaurants, cafés, and shopping streets, Ocean Suite is an excellent choice for travelers seeking a central studio in Athens with a modern design and easy access to the city's highlights."
		=> 'Κοντά στα κύρια αξιοθέατα, εστιατόρια, καφέ και εμπορικούς δρόμους, η Σουίτα Ocean είναι εξαιρετική επιλογή για κεντρικό studio με σύγχρονη σχεδίαση.',

	'Ginger Suite is a compact studio in Athens city center, located in the historic heart of the capital. Ideal for couples or solo travelers, this 17m² Athens accommodation offers a practical, comfortable, and well-located stay for short city breaks.'
		=> 'Η Σουίτα Ginger είναι ένα compact studio στην καρδιά της Αθήνας. Ιδανική για ζευγάρια ή μεμονωμένους ταξιδιώτες, αυτή η 17τ.μ. παραμονή προσφέρει πρακτική, άνετη και καλά τοποθετημένη παραμονή για σύντομες αστικές αποδράσεις.',
	'The studio features a comfortable double bed, a small seating area with sofa, and a functional layout designed for convenience and efficiency. Warm tones inspired by natural ginger shades create a cozy and inviting atmosphere, perfect for relaxing after exploring Athens.'
		=> 'Διπλό κρεβάτι, μικρό καθιστικό με καναπέ και λειτουργική διαρρύθμιση για βολικότητα και αποδοτικότητα. Ζεστές αποχρώσεις εμπνευσμένες από τον φυσικό τζίντζερ δημιουργούν άνετη και φιλόξενη ατμόσφαιρα.',
	'Guests can enjoy a fully equipped kitchen, a modern bathroom with shower, smart TV access, high-speed WiFi, and all essential amenities needed for a comfortable stay.'
		=> 'Πλήρως εξοπλισμένη κουζίνα, σύγχρονο μπάνιο με ντους, Smart TV, γρήγορο Wi-Fi και όλες οι απαραίτητες παροχές.',
	'Located within walking distance of Athens\' main attractions, restaurants, cafés, and shopping streets, Ginger Suite is an excellent choice for travelers seeking an affordable and centrally located studio in Athens with everything needed for a pleasant stay.'
		=> 'Κοντά στα κύρια αξιοθέατα, εστιατόρια, καφέ και εμπορικούς δρόμους, η Σουίτα Ginger είναι εξαιρετική επιλογή για οικονομικό και κεντρικό studio με όλα τα απαραίτητα.',

	'Grey Suite is a spacious studio in Athens city center, located in the historic heart of the capital. Offering 32m² of living space, this Athens accommodation is ideal for couples or solo travelers seeking extra comfort and room during their stay.'
		=> 'Η Σουίτα Gray είναι ένα ευρύχωρο studio στην καρδιά της Αθήνας. Με 32τ.μ. χώρο διαβίωσης, είναι ιδανική για ζευγάρια ή μεμονωμένους ταξιδιώτες που αναζητούν επιπλέον άνεση και χώρο.',
	'The studio features a comfortable double bed, a seating area with sofa, and a clean, contemporary design with neutral gray tones that create a calm and balanced atmosphere. Its larger layout provides added comfort compared to standard city studios.'
		=> 'Διπλό κρεβάτι, χώρος καθιστικού με καναπέ και σύγχρονη σχεδίαση με ουδέτερες γκρι αποχρώσεις που δημιουργούν ήρεμη και ισορροπημένη ατμόσφαιρα. Η μεγαλύτερη διαρρύθμιση προσφέρει επιπλέον άνεση.',
	'Guests can enjoy a fully equipped kitchen, a modern bathroom with shower, smart TV access, high-speed WiFi, and all essential amenities for a convenient short or extended stay in Athens.'
		=> 'Πλήρως εξοπλισμένη κουζίνα, σύγχρονο μπάνιο με ντους, Smart TV, γρήγορο Wi-Fi και όλες οι απαραίτητες παροχές για σύντομη ή παρατεταμένη παραμονή.',
	"Located within walking distance of Athens' main attractions, restaurants, cafés, and shopping streets, Gray Suite is a perfect choice for travelers looking for a spacious and centrally located studio in Athens with modern comfort and functionality."
		=> 'Κοντά στα κύρια αξιοθέατα, εστιατόρια, καφέ και εμπορικούς δρόμους, η Σουίτα Gray είναι ιδανική επιλογή για ευρύχωρο και κεντρικό studio με σύγχρονη άνεση.',

	'Sunshine Suite is a bright studio in Athens city center, located in the historic heart of the capital. This 15m² Athens accommodation is ideal for couples or solo travelers seeking a functional, comfortable, and centrally located stay.'
		=> 'Η Σουίτα Sunshine είναι ένα φωτεινό studio στην καρδιά της Αθήνας. Αυτή η 15τ.μ. παραμονή είναι ιδανική για ζευγάρια ή μεμονωμένους ταξιδιώτες που αναζητούν λειτουργική, άνετη και κεντρική παραμονή.',
	'The studio features a comfortable double bed, a small seating area with sofa, and a bright interior with warm tones that create a welcoming and uplifting atmosphere, perfect for relaxing after exploring Athens.'
		=> 'Διπλό κρεβάτι, μικρό καθιστικό με καναπέ και φωτεινό εσωτερικό με ζεστές αποχρώσεις που δημιουργούν φιλόξενη και ανεβαστική ατμόσφαιρα για χαλάρωση μετά την εξερεύνηση.',
	'Located within walking distance of Athens\' main attractions, restaurants, cafés, and shopping areas, Sunshine Suite is a practical and well-positioned choice for travelers looking for a central studio in Athens with comfort and easy access to the city highlights.'
		=> 'Κοντά στα κύρια αξιοθέατα, εστιατόρια, καφέ και εμπορικές περιοχές, η Σουίτα Sunshine είναι πρακτική και καλά τοποθετημένη επιλογή για κεντρικό studio στην Αθήνα.',

	'Forest Suite is a spacious apartment in Athens city center, located in the historic heart of the capital. Ideal for families, couples, or small groups of up to 4 guests, this 35m² Athens accommodation offers comfort, space, and a central location.'
		=> 'Η Σουίτα Forest είναι ένα ευρύχωρο διαμέρισμα στην καρδιά της Αθήνας. Ιδανική για οικογένειες, ζευγάρια ή μικρές παρέες έως 4 ατόμων, αυτή η 35τ.μ. παραμονή προσφέρει άνεση, χώρο και κεντρική τοποθεσία.',
	'The apartment features a double bed and a sofa bed, providing flexible sleeping arrangements for families or groups. The larger layout offers extra space compared to standard studios, making it ideal for longer stays or guests who prefer more comfort.'
		=> 'Διπλό κρεβάτι και καναπές-κρεβάτι για ευέλικτες διατάξεις ύπνου. Η πιο ευρύχωρη διαρρύθμιση σε σύγκριση με τυπικά studios την καθιστά ιδανική για παρατεταμένες παραμονές.',
	'Soft green tones inspired by nature create a calm and welcoming atmosphere throughout the space. Guests can enjoy a fully equipped kitchen, a modern bathroom with shower, smart TV access, high-speed WiFi, and all essential amenities for a convenient stay in Athens.'
		=> 'Απαλές πράσινες αποχρώσεις εμπνευσμένες από τη φύση δημιουργούν ήρεμη και φιλόξενη ατμόσφαιρα. Πλήρως εξοπλισμένη κουζίνα, σύγχρονο μπάνιο με ντους, Smart TV, γρήγορο Wi-Fi και όλες οι απαραίτητες παροχές.',
	"Located within walking distance of Athens' main attractions, restaurants, cafés, and shopping streets, Forest Suite is an excellent choice for travelers seeking a spacious and centrally located apartment in Athens with a relaxing, nature-inspired design."
		=> 'Κοντά στα κύρια αξιοθέατα, εστιατόρια, καφέ και εμπορικούς δρόμους, η Σουίτα Forest είναι εξαιρετική επιλογή για ευρύχωρο και κεντρικό διαμέρισμα στην Αθήνα με φυσική σχεδίαση.',

	/* ── Cookie Policy ───────────────────────────────────────────────── */
	'Cookie Policy'             => 'Πολιτική Cookies',
	'Last updated: May 2026'    => 'Τελευταία ενημέρωση: Μάιος 2026',
	'Cookies are small pieces of data stored in your browser and enable a website to \'remember\' your actions or preferences over time. Most browsers support cookies, but users can set their browsers to decline them and can delete them whenever they like.'
		=> 'Τα cookies είναι μικρά αρχεία δεδομένων που αποθηκεύονται στον browser σας και επιτρέπουν σε έναν ιστότοπο να «θυμάται» τις ενέργειες ή τις προτιμήσεις σας. Οι περισσότεροι browsers υποστηρίζουν cookies, αλλά μπορείτε να τα απενεργοποιήσετε ή να τα διαγράψετε οποτεδήποτε.',
	'How We Use Cookies'        => 'Πώς Χρησιμοποιούμε τα Cookies',
	'Our website utilises cookies which serve various purposes, but are generally either necessary or essential to the functioning of our site and help us improve the performance of our site. More specifically:'
		=> 'Ο ιστότοπός μας χρησιμοποιεί cookies για διάφορους σκοπούς, κυρίως απαραίτητα για τη λειτουργία του και τη βελτίωση της απόδοσής του. Πιο συγκεκριμένα:',
	'Strictly Necessary or Essential:'
		=> 'Απολύτως Αναγκαία:',
	"These cookies let you move around the website and use essential features. Please note that these cookies do not gather any information about you that could be used for marketing or remembering where you've been on the internet."
		=> 'Αυτά τα cookies σάς επιτρέπουν να περιηγείστε στον ιστότοπο και να χρησιμοποιείτε βασικές λειτουργίες. Δεν συλλέγουν πληροφορίες για εμπορικούς σκοπούς.',
	'Performance:'              => 'Απόδοση:',
	'These cookies collect information about how you use our website, for example which pages you visit, and if you experienced any errors. Performance cookies do not collect any information that could identify you. They are only used to help us improve how our website works, understand what interests our users and measure how effective our content is by providing anonymous statistics and data regarding how our website is used. Accepting these cookies is a condition of using our site.'
		=> 'Αυτά τα cookies συλλέγουν πληροφορίες για τον τρόπο χρήσης του ιστότοπου (π.χ. ποιες σελίδες επισκέπτεστε). Δεν σας αναγνωρίζουν ατομικά. Χρησιμοποιούνται μόνο για τη βελτίωση της λειτουργίας μας, με ανώνυμα στατιστικά. Η αποδοχή τους αποτελεί προϋπόθεση χρήσης του ιστότοπου.',
	'Functionality:'            => 'Λειτουργικότητα:',
	'These cookies are used to provide services or to remember settings to improve your visit.'
		=> 'Αυτά τα cookies αποθηκεύουν ρυθμίσεις για τη βελτίωση της επίσκεψής σας.',
	'Managing Your Cookies'     => 'Διαχείριση των Cookies',
	"The cookie settings on this website are set to 'accept' to give you the best browsing experience possible. If you continue to use this website without changing your cookie settings then you are consenting to their use as described herein and in our Privacy Policy."
		=> 'Οι ρυθμίσεις cookies σε αυτόν τον ιστότοπο είναι από προεπιλογή σε «αποδοχή» για τη βέλτιστη εμπειρία περιήγησης. Εάν συνεχίσετε χωρίς να αλλάξετε τις ρυθμίσεις cookies, συναινείτε στη χρήση τους όπως περιγράφεται εδώ και στην Πολιτική Απορρήτου.',
	'When accepted, cookies remain until they expire or until you clear your browser. To delete cookies or instruct your web browser to delete or refuse cookies, please visit the documentation pages of your web browser. Please note, however, that if you delete cookies or refuse to accept them, you might not be able to use all of the features we offer, you may not be able to store your preferences, and some of our pages might not display properly.'
		=> 'Όταν γίνουν αποδεκτά, τα cookies παραμένουν μέχρι να λήξουν ή να καθαρίσετε τον browser. Για διαγραφή ή άρνηση cookies, ανατρέξτε στην τεκμηρίωση του browser σας. Σημειώστε ότι η άρνηση cookies ενδέχεται να επηρεάσει ορισμένες λειτουργίες του ιστότοπου.',
	"Internet browsers allow you to change your cookie settings. These settings are usually found in the 'options' or 'preferences' menu of your internet browser. In order to understand these settings, the following links may be helpful:"
		=> 'Οι browsers επιτρέπουν την αλλαγή ρυθμίσεων cookies, συνήθως στο μενού «επιλογές» ή «προτιμήσεις». Οι παρακάτω σύνδεσμοι μπορεί να σας βοηθήσουν:',
	'Contact'                   => 'Επικοινωνία',
	'If you wish to withdraw your consent at any time, you will need to delete your cookies using your internet browser settings.'
		=> 'Εάν επιθυμείτε να ανακαλέσετε τη συγκατάθεσή σας ανά πάσα στιγμή, θα χρειαστεί να διαγράψετε τα cookies μέσω των ρυθμίσεων του browser σας.',
	'We hope the information above provides you with clear and comprehensive information about the technologies we use and the purposes for which we use them. If you have any additional questions or require any additional information, please review our'
		=> 'Ελπίζουμε ότι οι παραπάνω πληροφορίες σάς παρέχουν σαφή και πλήρη εικόνα για τις τεχνολογίες που χρησιμοποιούμε. Εάν έχετε επιπλέον ερωτήσεις, παρακαλούμε ελέγξτε την',
	'or contact us at'          => 'ή επικοινωνήστε μαζί μας στο',

	/* ── Terms & Conditions ──────────────────────────────────────────── */
	'Terms & Conditions'        => 'Όροι & Προϋποθέσεις',
	'Arrival and Departure'     => 'Άφιξη και Αναχώρηση',
	'Check-in starts at 14:00 on your arrival day. Check-out: 11:00 AM on the departure day.'
		=> 'Το check-in ξεκινά στις 14:00 της ημέρας άφιξης. Check-out: 11:00 π.μ. την ημέρα αναχώρησης.',
	'We offer you an easy and secure key-less access to your suite. This access allows you to check-in without our physical presence. Prior to your arrival, you will receive a unique access code corresponding to your reservation. This code will be valid for the duration of your stay.'
		=> 'Σας προσφέρουμε εύκολη και ασφαλή πρόσβαση χωρίς κλειδί. Αυτή η πρόσβαση σας επιτρέπει να κάνετε check-in χωρίς τη φυσική μας παρουσία. Πριν από την άφιξή σας θα λάβετε έναν μοναδικό κωδικό πρόσβασης που αντιστοιχεί στην κράτησή σας και θα ισχύει για όλη τη διάρκεια της παραμονής σας.',
	'Each guest has their personal code for the apartment, as well as the code for building and floor access sent around 14:00 or even earlier if permitted by availability. The ground floor door is locked after 18:00 — if you arrive later, use the code provided.'
		=> 'Κάθε επισκέπτης λαμβάνει προσωπικό κωδικό για το διαμέρισμα, καθώς και κωδικό για το κτίριο και τον όροφο, αποστέλλεται περίπου στις 14:00 ή νωρίτερα εάν το επιτρέπει η διαθεσιμότητα. Η πόρτα του ισογείου κλειδώνει μετά τις 18:00 — αν φτάσετε αργότερα, χρησιμοποιήστε τον κωδικό που σας εστάλη.',
	'In case you need our physical presence, you can find us daily at our office from 9:00 AM to 16:00 on the ground floor of 11 Theseos Street.'
		=> 'Αν χρειάζεστε τη φυσική μας παρουσία, θα μας βρείτε καθημερινά στο γραφείο μας από τις 9:00 έως τις 16:00, στο ισόγειο της οδού Θησέως 11.',
	'Reservations'              => 'Κρατήσεις',
	'Guests can cancel free of charge 7 days prior to arrival. The guest will be charged the total reservation amount if cancelled within 7 days of arrival.'
		=> 'Οι επισκέπτες μπορούν να ακυρώσουν δωρεάν έως 7 ημέρες πριν από την άφιξη. Σε περίπτωση ακύρωσης εντός 7 ημερών από την άφιξη, θα χρεωθεί το σύνολο της κράτησης.',
	"If the guest doesn't show up, the total reservation amount will be charged. Full payment for the reservation will be charged to the credit or debit card provided by the customer."
		=> 'Σε περίπτωση μη εμφάνισης, χρεώνεται το σύνολο της κράτησης. Πλήρης πληρωμή θα χρεωθεί στην πιστωτική ή χρεωστική κάρτα που έχει δηλώσει ο πελάτης.',
	'Cancellation by Us'        => 'Ακύρωση από Εμάς',
	'If we need to cancel a reservation, we will contact you immediately. Any payments will be fully refunded.'
		=> 'Εάν χρειαστεί να ακυρώσουμε μια κράτηση, θα επικοινωνήσουμε άμεσα μαζί σας. Οποιεσδήποτε πληρωμές θα επιστραφούν πλήρως.',
	'Room Change'               => 'Αλλαγή Δωματίου',
	'If a reservation is made for a specific room and that reservation for that room cannot be used due to circumstances beyond our control, we reserve the right to transfer the reservation to a same type room.'
		=> 'Εάν γίνει κράτηση για συγκεκριμένο δωμάτιο και δεν μπορεί να χρησιμοποιηθεί λόγω αναπότρεπτων περιστάσεων, διατηρούμε το δικαίωμα να μεταφέρουμε την κράτηση σε δωμάτιο του ίδιου τύπου.',
	'Prices'                    => 'Τιμές',
	'Prices include 13% VAT &amp; City tax 0.5% per booking. Environmental tax of €0.50 for the period from 01/11 to 28/02 and €1.50 for the period from 01/03 to 31/10 per night is NOT included.'
		=> 'Οι τιμές περιλαμβάνουν ΦΠΑ 13% &amp; φόρο πόλης 0,5% ανά κράτηση. Δεν περιλαμβάνεται περιβαλλοντικό τέλος €0,50 για την περίοδο 01/11–28/02 και €1,50 για την περίοδο 01/03–31/10 ανά διανυκτέρευση.',
	'Prices include 13% VAT & City tax 0.5% per booking. Environmental tax of €0.50 for the period from 01/11 to 28/02 and €1.50 for the period from 01/03 to 31/10 per night is NOT included.'
		=> 'Οι τιμές περιλαμβάνουν ΦΠΑ 13% & φόρο πόλης 0,5% ανά κράτηση. Δεν περιλαμβάνεται περιβαλλοντικό τέλος €0,50 για την περίοδο 01/11–28/02 και €1,50 για την περίοδο 01/03–31/10 ανά διανυκτέρευση.',
	'Parking'                   => 'Στάθμευση',
	'No parking is available for guests on the premises. Private paid parking is available nearby. Parking is your responsibility.'
		=> 'Δεν υπάρχει χώρος στάθμευσης για επισκέπτες στις εγκαταστάσεις. Ιδιωτικός επί πληρωμή χώρος στάθμευσης διατίθεται κοντά. Η στάθμευση είναι ευθύνη σας.',
	'Damages'                   => 'Ζημιές',
	'You are responsible for any damages to the property or its contents. We reserve the right to charge for repairs or replacements if the damage is significant. We also reserve the right to charge for missing items.'
		=> 'Είστε υπεύθυνοι για τυχόν ζημιές στο κατάλυμα ή στα περιεχόμενά του. Διατηρούμε το δικαίωμα να χρεώσουμε για επισκευές ή αντικαταστάσεις εφόσον η ζημιά είναι σημαντική, καθώς και για ελλείποντα αντικείμενα.',
	'Liability'                 => 'Ευθύνη',
	'We are not liable for any damage, loss, or injury throughout your stay on the premises.'
		=> 'Δεν φέρουμε ευθύνη για τυχόν ζημιές, απώλειες ή τραυματισμούς κατά τη διάρκεια της παραμονής σας.',
	'Lost Items'                => 'Χαμένα Αντικείμενα',
	'If items are found left after your departure, we will return them. However, we may charge for shipping to cover postage and packaging.'
		=> 'Εάν βρεθούν αντικείμενα μετά την αναχώρησή σας, θα σας τα επιστρέψουμε. Ωστόσο, ενδέχεται να χρεωθείτε τα έξοδα αποστολής.',
	'Room Cleaning Service'     => 'Υπηρεσία Καθαρισμού',
	'Towels will be changed every 4 days. However, we also offer the following options for your convenience:'
		=> 'Οι πετσέτες αλλάζουν κάθε 4 ημέρες. Επίσης, για τη διευκόλυνσή σας, προσφέρουμε τις εξής επιλογές:',
	'Room Cleaning: Available upon request for an additional cost of €30 per day. This service includes general cleaning of the room, fresh bed sheets, and towels.'
		=> 'Καθαρισμός Δωματίου: Διαθέσιμος κατόπιν αιτήματος με επιπλέον χρέωση €30 ανά ημέρα. Περιλαμβάνει γενικό καθαρισμό, αλλαγή σεντονιών και πετσετών.',
	'Extra Towels: Guests may request an additional set of fresh bath towels for an extra cost of €5 per set.'
		=> 'Επιπλέον Πετσέτες: Οι επισκέπτες μπορούν να ζητήσουν επιπλέον σετ μπάνιου με επιπλέον χρέωση €5 ανά σετ.',
	'Pets'                      => 'Κατοικίδια',
	'Pets are not permitted within the premises.'
		=> 'Δεν επιτρέπονται κατοικίδια στις εγκαταστάσεις.',
	'Social Gatherings and Events' => 'Κοινωνικές Συναθροίσεις και Εκδηλώσεις',
	'The hosting of parties or events within the building is strictly prohibited.'
		=> 'Η διοργάνωση πάρτι ή εκδηλώσεων εντός του κτιρίου απαγορεύεται αυστηρά.',
	'Visitor Prohibition Policy' => 'Πολιτική Απαγόρευσης Επισκεπτών',
	'For the safety and privacy of all our guests, we do not allow access to the property for visitors who are not included in the initial reservation during your stay. Only registered guests are allowed on the premises.'
		=> 'Για την ασφάλεια και την ιδιωτικότητα όλων των επισκεπτών μας, δεν επιτρέπουμε πρόσβαση στο κατάλυμα σε επισκέπτες που δεν περιλαμβάνονται στην αρχική κράτηση. Μόνο εγγεγραμμένοι επισκέπτες επιτρέπονται στις εγκαταστάσεις.',
	'Unaccompanied Minors'      => 'Ανήλικοι χωρίς Συνοδό',
	'Individuals under the age of 18 must be accompanied by an adult guardian to access the premises. Unaccompanied minors are not granted entry.'
		=> 'Άτομα κάτω των 18 ετών πρέπει να συνοδεύονται από ενήλικα κηδεμόνα. Ανήλικοι χωρίς συνοδό δεν γίνονται δεκτοί.',
	'Smoking'                   => 'Κάπνισμα',
	'Smoking is permitted exclusively in designated smoking areas.'
		=> 'Το κάπνισμα επιτρέπεται αποκλειστικά σε ειδικά καθορισμένους χώρους.',
	'Termination Policy'        => 'Πολιτική Διακοπής Παραμονής',
	'Chic Center Suites Athens reserves the right, at its discretion, to terminate the stay of an individual without warning if deemed necessary due to unacceptable behavior or actions that may endanger the property or offend others. In such cases, no refunds will be issued.'
		=> 'Τα Chic Centre Suites Αθήνα διατηρούν το δικαίωμα, κατά την κρίση τους, να διακόψουν τη διαμονή ατόμου χωρίς προειδοποίηση, εφόσον κριθεί απαραίτητο λόγω απαράδεκτης συμπεριφοράς ή ενεργειών που ενδέχεται να θέσουν σε κίνδυνο την περιουσία ή να προσβάλουν τρίτους. Σε τέτοιες περιπτώσεις δεν εκδίδονται επιστροφές χρημάτων.',
	'We reserve the right to modify these terms and conditions at any time.'
		=> 'Διατηρούμε το δικαίωμα τροποποίησης των παρόντων όρων και προϋποθέσεων ανά πάσα στιγμή.',
	'By booking one of our suites, you acknowledge and agree to the above.'
		=> 'Με την κράτηση μιας σουίτας μας, αποδέχεστε και συμφωνείτε με τα παραπάνω.',

	/* ── Privacy Policy (key headings + select paragraphs) ───────────── */
	'Privacy Policy'            => 'Πολιτική Απορρήτου',
	'Introduction'              => 'Εισαγωγή',
	'Purpose of This Privacy Notice'    => 'Σκοπός της Παρούσας Πολιτικής',
	'Controller'                => 'Υπεύθυνος Επεξεργασίας',
	'Contact Details'           => 'Στοιχεία Επικοινωνίας',
	'Changes to the Privacy Notice'     => 'Αλλαγές στην Πολιτική Απορρήτου',
	'Third-Party Links'         => 'Σύνδεσμοι Τρίτων',
	'The Data We Collect About You'     => 'Τα Δεδομένα που Συλλέγουμε',
	'If You Fail to Provide Personal Data'  => 'Αν Δεν Παρέχετε Προσωπικά Δεδομένα',
	'How Is Your Personal Data Collected?'  => 'Πώς Συλλέγουμε τα Προσωπικά σας Δεδομένα;',
	'How We Use Your Personal Data'     => 'Πώς Χρησιμοποιούμε τα Δεδομένα σας',
	'Purposes for Which We Will Use Your Personal Data' => 'Σκοποί Χρήσης Προσωπικών Δεδομένων',
	'Marketing'                 => 'Μάρκετινγκ',
	'Cookies'                   => 'Cookies',
	'Change of Purpose'         => 'Αλλαγή Σκοπού',
	'Disclosures of Your Personal Data' => 'Κοινοποίηση Προσωπικών Δεδομένων',
	'International Transfers'   => 'Διεθνείς Μεταβιβάσεις',
	'Data Security'             => 'Ασφάλεια Δεδομένων',
	'Data Retention'            => 'Διατήρηση Δεδομένων',
	'Your Legal Rights'         => 'Τα Νόμιμα Δικαιώματά σας',
	'Your Legal Rights (Detail)'        => 'Τα Νόμιμα Δικαιώματά σας (Λεπτομέρειες)',
	'You have the right to:'    => 'Έχετε το δικαίωμα να:',
	'Our full details are:'     => 'Τα πλήρη στοιχεία μας:',
	'Glossary'                  => 'Γλωσσάριο',
	'Lawful Basis'              => 'Νόμιμη Βάση',
	'External Third Parties'    => 'Εξωτερικοί Τρίτοι',
	'Changes to Our Privacy Policy'     => 'Αλλαγές στην Πολιτική μας',
	'Purpose / Activity'        => 'Σκοπός / Δραστηριότητα',
	'Type of data'              => 'Τύπος δεδομένων',
	'Lawful basis for processing'       => 'Νόμιμη βάση επεξεργασίας',
	'Full name of legal entity:'        => 'Πλήρης επωνυμία νομικής οντότητας:',
	'Email address:'            => 'Διεύθυνση email:',
	'Postal address:'           => 'Ταχυδρομική διεύθυνση:',
	'Telephone number:'         => 'Τηλέφωνο:',

	/* ── Terms & Conditions (split-label variants) ──────────────────── */
	'Available upon request for an additional cost of €30 per day. This service includes general cleaning of the room, fresh bed sheets, and towels.'
		=> 'Διαθέσιμος κατόπιν αιτήματος με επιπλέον χρέωση €30 ανά ημέρα. Περιλαμβάνει γενικό καθαρισμό, αλλαγή σεντονιών και πετσετών.',
	'Guests may request an additional set of fresh bath towels for an extra cost of €5 per set.'
		=> 'Οι επισκέπτες μπορούν να ζητήσουν επιπλέον σετ μπάνιου με επιπλέον χρέωση €5 ανά σετ.',

];
